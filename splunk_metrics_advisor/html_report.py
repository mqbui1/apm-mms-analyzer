...
<div class="page">
  ...
  <p style="margin-top:32px;color:var(--subtle);font-size:11px">
    Generated by splunk-metrics-advisor &mdash; Powered by Claude via AWS Bedrock
  </p>
</div>
<script>{_JS}</script>
</body>
...
# ---------------------------------------------------------------------------
# Custom metrics HTML report
# ---------------------------------------------------------------------------

def generate_custom(analysis: dict, ai_text: str, generated_at: str = "") -> str:
    """Build the full HTML report for custom (IM) metrics cardinality analysis."""
    total_mts     = analysis["total_mts"]
    total_metrics = analysis["total_metrics"]
    reduction_est = analysis["reduction_estimate"]
    reduction_pct = analysis["reduction_pct"]
    cost          = total_mts * 0.002
    culprits      = analysis["cardinality_culprits"]

    # ── Stat grid ─────────────────────────────────────────────────────────────
    sev_color = "red" if total_mts >= 2_000_000 else "orange" if total_mts >= 800_000 else "yellow"
    stats_html = f"""<div class="stat-grid">
  {_stat_box(f"{total_mts:,}", "Total MTS", sev_color)}
  {_stat_box(f"{total_metrics:,}", "Custom Metrics", "")}
  {_stat_box(f"${cost:,.2f}", "Est. $/mo", "purple")}
  {_stat_box(f"{reduction_est:,}", "MTS Reducible", "green")}
  {_stat_box(f"{reduction_pct}%", "Reduction Potential", "orange")}
  {_stat_box(f"{len(culprits)}", "Cardinality Culprits", "red" if culprits else "green")}
</div>"""

    # ── Top metrics table ─────────────────────────────────────────────────────
    top = analysis["top_metrics"][:30]
    if top:
        rows = []
        for i, m in enumerate(top, 1):
            pct_val = round(m["mts_count"] / max(total_mts, 1) * 100, 1)
            bar_w = max(1, int(pct_val * 3))
            rows.append(f"""<tr>
  <td class="rank">{i}</td>
  <td><code class="pat-code">{html.escape(m['metric_name'])}</code></td>
  <td style="text-align:right"><strong>{m['mts_count']:,}</strong></td>
  <td style="text-align:right">{pct_val}%</td>
  <td><div style="background:var(--accent);height:8px;border-radius:4px;width:{bar_w}px;opacity:.6"></div></td>
</tr>""")
        top_metrics_html = f"""<table class="patterns-table">
<thead><tr>
  <th>#</th><th>Metric Name</th>
  <th style="text-align:right">MTS Count</th>
...
<tbody>{"".join(rows)}</tbody>
        ...
        top_metrics_html = "<p style='color:var(--muted)'>No metrics data.</p>"
    ...
    if culprits:
        ...
        for c in culprits:
            affected = ", ".join(
                f"<code style='font-size:11px'>{html.escape(m)}</code>"
                for m in c["affected_metrics"][:3]
            )
            if c["affected_metric_count"] > 3:
                affected += f" <span style='color:var(--muted)'>+{c['affected_metric_count']-3}</span>"
            culprit_rows.append(f"""<tr class="attack-row">
  <td><code style="font-size:12px">{html.escape(c['dimension'])}</code></td>
  <td style="text-align:right"><strong>{c['total_mts']:,}</strong></td>
  <td style="text-align:right">{c['pct_of_total']}%</td>
  <td style="font-size:12px;color:var(--muted)">{html.escape(c['reason'])}</td>
  <td style="font-size:12px">{affected}</td>
</tr>""")
        culprits_html = f"""<table class="patterns-table">
<thead><tr>
  <th>Dimension</th>
  <th style="text-align:right">MTS Contributed</th>
  <th style="text-align:right">% Total</th>
  <th>Why It Is High-Cardinality</th>
  <th>Affected Metrics</th>
</tr></thead>
<tbody>{"".join(culprit_rows)}</tbody>
</table>"""
    else:
        culprits_html = "<p style='color:var(--green)'>No high-cardinality dimensions detected.</p>"

    # ── Metric groups table ───────────────────────────────────────────────────
    if analysis["metric_groups"]:
        group_rows = []
        for g in analysis["metric_groups"]:
            bar_w = max(1, int(g["pct"] * 2))
            group_rows.append(f"""<tr>
  <td><code class="pat-code">{html.escape(g['prefix'])}.*</code></td>
  <td style="text-align:right">{g['metric_count']}</td>
  <td style="text-align:right"><strong>{g['mts_count']:,}</strong></td>
  <td style="text-align:right">{g['pct']}%</td>
  <td><div style="background:var(--accent);height:8px;border-radius:4px;width:{bar_w}px;opacity:.6"></div></td>
</tr>""")
        groups_html = f"""<table class="patterns-table">
<thead><tr>
  <th>Group</th><th style="text-align:right"># Metrics</th>
  <th style="text-align:right">Total MTS</th><th style="text-align:right">%</th>
  <th>Distribution</th>
</tr></thead>
<tbody>{"".join(group_rows)}</tbody>
</table>"""
    else:
        groups_html = "<p style='color:var(--muted)'>No group data.</p>"

    # ── Token distribution table ──────────────────────────────────────────────
    if analysis["token_distribution"]:
        token_rows = []
        for t in analysis["token_distribution"]:
            top3 = ", ".join(
                f"<code style='font-size:11px'>{html.escape(name)}</code>"
                for name, _ in t["top_metrics"][:3]
            )
            token_rows.append(f"""<tr>
  <td><code style="font-size:12px">{html.escape(t['token_id'])}</code></td>
  <td style="text-align:right"><strong>{t['mts_count']:,}</strong></td>
  <td style="text-align:right">{t['pct']}%</td>
  <td style="font-size:12px">{top3}</td>
</tr>""")
        tokens_html = f"""<table class="patterns-table">
<thead><tr>
  <th>Token / Account ID</th>
  <th style="text-align:right">MTS Count</th>
  <th style="text-align:right">% Total</th>
  <th>Top Metrics</th>
</tr></thead>
<tbody>{"".join(token_rows)}</tbody>
        ...
        tokens_html = "<p style='color:var(--muted)'>No token data available.</p>"

    # ── AI analysis content ───────────────────────────────────────────────────
    ai_html = f'<div class="ai-content">{_md_to_html(ai_text.strip())}</div>'

    # ── Assemble ──────────────────────────────────────────────────────────────
    body = f"""
{stats_html}
{_card("High-Cardinality Dimension Culprits", culprits_html, "sec-culprits",
       border_color="#ef4444" if culprits else "")}
{_card("AI Analysis", ai_html, "sec-ai", border_color="#3b82f6")}
{_card("Top Metrics by MTS Count", top_metrics_html, "sec-top", open_by_default=False)}
{_card("Metric Groups", groups_html, "sec-groups", open_by_default=False)}
{_card("Token / Account Distribution", tokens_html, "sec-tokens", open_by_default=False)}
"""

    meta = f"Generated {generated_at}" if generated_at else ""
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Custom Metrics Cardinality Report</title>
<style>{_CSS}</style>
</head>
<body>
<div class="page">
  <div class="report-header">
    <button class="theme-btn" onclick="toggleTheme()">Toggle theme</button>
    <h1>Custom Metrics Cardinality Report</h1>
    <div class="meta">{meta}</div>
  </div>
  {body}
  <p style="margin-top:32px;color:var(--subtle);font-size:11px">
